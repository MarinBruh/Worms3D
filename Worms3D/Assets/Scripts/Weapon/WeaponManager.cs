...
using TMPro;
public class WeaponManager : PlayerSwitcher
{
    //GameObjects
    [Header("GameObjects")]
    ...
    void Update()
    {
        if (GetComponent<PlayerController>().playerActive && PlayerSwitcher.GM.State == GameState.Movement)
        {     
            InputManager();
            ChargeHandler();
        ...
        if (PlayerSwitcher.GM.State != GameState.Movement)
        {
            CrossHairOff();
        }
    ...
    {
        GetComponent<PlayerController>().velocity = -transform.forward * 5 * Time.deltaTime;
    }
